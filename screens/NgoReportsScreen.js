...
export default class NgoReportsScreen extends React.Component {
    constructor() {
        super();
        this.state = {
            reports: [],
    ...
    async componentDidMount() {
        let data = await getPursuedReports(this.state.email)
        this.setState({
            reports: data[0]
        })
        this.arrayholder = data[0]
    }
    ...
    renderItem = ({ item, index }) => {
        return (
            ...
            onDelete={async()=>{
                this.setState({
                    reports: await deleteReport(item.report_id, 'pursuer', this.state.email, this.state.sorter)
                })
                this.arrayholder = await deleteReport(item.report_id, 'pursuer', this.state.email, this.state.sorter)
            }}
            deletable={true}/>
        )
    }
